...
class DynamicImage {
    ...
    get $selectedContainer() {
        return this.$container.find('.pl-dynamic-image__selected');
    }

    get $triggers() {
        return this.$container.find('.pl-dynamic-image__trigger');
    }
    ...
    swapContainerImage(newImageSrc, fullSwap = true) {
        if (!fullSwap && this.$img.length > 0) {
            ...
            img.classList.remove('pl-dynamic-image__active');
            setTimeout(() => img.classList.add('pl-dynamic-image__active'), 1);
        } else {
            const img = document.createElement('img');
            img.src = newImageSrc;
            img.classList.add('pl-dynamic-image__active');
...
/* SAMPLE MARKUP
<div class="pl-dynamic-image">
    <div class="pt-6 md:pt-12 flex">
        <div class="max-md:hidden pl-dynamic-image__selected w-1/4 h-[300px]"></div>

        <ul class="md:w-3/4 md:pl-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-4">
            <?php foreach ($children as $topic) : ?>
                <li class="pl-dynamic-image__trigger" data-image="<?php echo get_the_post_thumbnail_url($topic, 'large') ?>" data-id="<?php echo $topic->ID ?>">
